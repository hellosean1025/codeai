...
import { generateText, CoreMessage, Tool as CoreTool } from "ai";
import { z } from "zod";
...
async function convertToVercelAITools(
  tools: Tool[],
...
function convertToVercelMessages(messages: Message[]): CoreMessage[] {
...
async function executeTool(
  ...
  tools: Tool[],
  verbose: boolean,
): Promise<{ id: string; result: string; error?: boolean }> {
...
async function executeTools(
  ...
  if (allReadOnly && toolUses.length > 1) {
    ...
    return Promise.all(
      toolUses.map(tu => executeTool(tu, tools, verbose))
    );
  } else {
    if (verbose) {
