    ...
    execute: async (input) => {
        try {
            ...
            if (!input.replace_all) {
                ...
                if (occurrences > 1) {
                    throw new Error(`String appears ${occurrences} times in file. Use replace_all: true or provide more context to make it unique.`);
                }
            }
            ...
            throw new Error(`Failed to edit file ${input.file_path}: ${error instanceof Error ? error.message : String(error)}`);
        }
    },
};
