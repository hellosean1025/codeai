...
export const writeTool = {
    ...
    execute: async (input) => {
        ...
        try {
            ...
            // 检查是否在安全目录
            const isInAllowedPath = Array.from(allowedBasePaths).some(basePath => absolutePath.startsWith(basePath));
            if (!isInAllowedPath) {
                throw new Error(`File writing paths only allowed in: ${Array.from(allowedBasePaths).join(', ')}`);
            }
            ...
            throw new Error(`Failed to write file ${absolutePath}: ${error instanceof Error ? error.message : String(error)}`);
        }
    },
};
